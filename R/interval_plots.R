interval_plots <- function(
  ...
  stopifnot(
    any(alpha, beta, gammadelta, sm)
  )

  sm_text <- ifelse(sm, "_sm", "")

  # get params to summarise
  alpar <- ifelse(alpha,      "alpha",             NULL)
  ...
  sapar <- ifelse(sm,         "sampling",          NULL)

  sumparams <- c(alpar, bepar, gdpar, sapar)

  summa <- MCMCvis::MCMCsummary(
    draws,
    params = sumparams,
    exact = FALSE,
  ...
  df <- summa |>
    ...
    mutate(
      v1 = case_when(
        par == "alpha_mean" ~ "mean",
        par == "alpha_sd" ~ "sd",
        par == "alpha_raw" ~ target_species[idx1],
        par == "beta_mean" ~ "mean",
        par == "beta_sd" ~ "sd",
        par == "beta" ~ target_covariate_names[idx1],
        par == "gamma_mean" ~ "mean",
        par == "gamma_sd" ~ "sd",
        par == "gamma_raw" ~ target_species[idx1],
        par == "delta" ~ "intercept",
    ...
    select(par, parshort, v1, v2, everything(), -idx1, -idx2)
  ...
  if(alpha){
    ...
    df |>
      filter(parshort == "alpha") |>
      mutate(
        v1 = factor(
          v1,
          levels = c("mean", "sd", target_species) |>
            rev()
        )
      ) |>
      ggplot(aes(y = v1)) +
  ...
  if(gammadelta){
    df |>
      filter(parshort == "gamma" | par == "delta") |>
      mutate(
        v1 = factor(
          v1,
          levels = c("intercept", "mean", "sd", target_species) |>
            rev()
        )
      ) |>
      ggplot(aes(y = v1)) +
