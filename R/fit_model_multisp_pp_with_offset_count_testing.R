fit_model_multisp_pp_with_offset_count <- function(
  ...
  # model_data_ragged <- model_data_ragged |>
  #   select(lon,lat,type, arabiensis, gambiae, funestus)
  ...
  # add on a `cell_row` to notna_idx_pa and then only index out the
  # repeated data
  data_index <- notna_idx_pa[, c("data_row", "species")]

  spatial_index <- notna_idx_pa[, c("cell_row", "species")]

  distribution(data_infilled[data_index]) <- poisson(lambda[spatial_index])

  #distribution(data_infilled[model_notna_idx_pa]) <- poisson(pa_rate)


  # distribution(obs$y) <- poisson(lambda[cbind(obs$rows, obs$species)])
  ...
  #
  #
  # po_records_i,s ~ Poisson(lambda_i,s * bias_i,s)
  # pa_i,s ~ Bernoulli(prob_pres_i,s)
  # count_i,s ~  Poisson(lambda_i,s * effort_i)
  #
  # log(bias_i,s) = gamma_s + delta * bias_cov_i
  #
  # prob_pres_i,s = icloglog(lambda_i,s * effort_i)
  #
  # for observation i and species s
  ...
  ## adaptive HMC branch of greta - as yet unreleased
  # might be more faster higher stronger
  # njtierney/adaptive-hmc-v3-i765 not on main greta dev branch
  #
