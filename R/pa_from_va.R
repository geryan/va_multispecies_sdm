pa_from_va <- function(va_data){
  va_data |>
    ...
    ungroup() |>
    select(source_ID, species, lon, lat, binary.presence, binary.absence, starts_with("n_"), count, presence, pa, starts_with("sampling.method_")) |>
...
#
# x <- va_data |>
#   filter(source_ID == 386, species == "GAMBIAE COMPLEX") %>%
#   select(year_st, lon, lat, starts_with("sampling.method_"), starts_with("n_")) |>
#   mutate(
#     any_sm_na_count = any(
#       !is.na(sampling.method_1) & is.na(n_1),
#       !is.na(sampling.method_2) & is.na(n_2),
...
#       !is.na(sampling.method_4) & is.na(n_4)
#     ), # this checks if there are any non-empty sampling methods with a NA count
#     all_sm_na = all(is.na(c_across(starts_with("sampling.method"))))
#   ) |>
